...
function () {
  ...
  _createClass(Utils, [{
    ...
    value: function mapSorter(map) {
      ...
      if (!this.isLeaf(map)) {
        _toConsumableArray(map.keys()).forEach(function (key) {
          if (key !== "@") {
            if (path) {
              finishpath = path + '/' + key;
            } else {
        ...
        map = new Map(_toConsumableArray(map.entries()).sort(function (a, b) {
    ...
    value: function removeItems(origData, dir, remove) {
      ...
      var data = _Storage["default"].xPathGetDataWithIterator(dir, origData);
      ...
      _Storage["default"].xPathGetDataWithIterator(dir, origData, newArray);
    }
  }, {
    ...
    value: function getAttributeValue(metaObj, fromPath, inputPathWIterator, transformations) {
      ...
      if (transformations) {
        transformations.forEach(function (transformation) {
          ...
          realValue = _Storage["default"].xPathGetDataFromStorage('functions/valueStorage/' + transformation.name).apply(void 0, [realValue, index].concat(_toConsumableArray(para)));
        });
      }
    ...
    */
    ...
    value: function insertIterators(fromPath, toPath, iteratorObj, data, fileName, output) {
      ...
      //build parentobj
      var iteratorKeys = _toConsumableArray(iteratorObj.keys());
